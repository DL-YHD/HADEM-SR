...
from utilities.utilities import calculate_psnr, calculate_ssim,tensor2img, calc_Freq
    ...
    def __evaluation__(self, eval_loader, eval_iter, epoch, step = 0):
        ...
        with torch.no_grad():
            for _ in tqdm(range(eval_iter)):
                ...
                # **************************** DEM Module *********************** #
                # =============================================================== #
                low_freq_gt, high_freq_gt = calc_Freq(hr,kernel=3) #low and high
                low_freq_gt     = tensor2img(low_freq_gt.cpu())
                high_freq_gt    = tensor2img(high_freq_gt.cpu())
                res = self.network(lr)
                low_freq_res, high_freq_res = calc_Freq(res,kernel=3) #low and high
                # res, low_res, high_res = self.network(lr)
                res = tensor2img(res.cpu())
                
                low_freq_res     = tensor2img(low_freq_res.cpu())
                high_freq_res    = tensor2img(high_freq_res.cpu())
                # res_aux = low_res + high_res # New added
                # res_aux = tensor2img(res_aux.cpu()) # New added
                ...
                # psnr_aux    = calculate_psnr(res_aux[0],hr[0])
                # ssim_aux    = calculate_ssim(res_aux[0],hr[0])

                psnr_aux_1    = calculate_psnr(low_freq_res[0],low_freq_gt[0])
                psnr_aux_2    = calculate_psnr(high_freq_res[0],high_freq_gt[0])
                ssim_aux_1    = calculate_ssim(low_freq_res[0],low_freq_gt[0])
                ssim_aux_2    = calculate_ssim(high_freq_res[0],high_freq_gt[0])

                total_psnr+= psnr
                total_ssim+= ssim

                # total_psnr_aux += psnr_aux
                # total_ssim_aux += ssim_aux

                total_psnr_aux += ((psnr_aux_1 + psnr_aux_2) / 2)
                total_ssim_aux += ((ssim_aux_1 + ssim_aux_2) / 2)
        ...
        final_psnr_aux = total_psnr_aux/total_num
        final_ssim_aux = total_ssim_aux/total_num
        ...
        print("[{}], Epoch [{}], Dataset: {}, PSNR: {:.4f}, SSIM: {:.4f}, AUX_PSNR: {:.4f}, AUX_SSIM: {:.4f}".format(self.config["version"], epoch, dataset_name, final_psnr, final_ssim, final_psnr_aux, final_ssim_aux))
        self.reporter.writeTrainLog(epoch,step, "Dataset: {}, PSNR: {:.4f}, SSIM: {:.4f}, AUX_PSNR: {:.4f}, AUX_SSIM: {:.4f}".format(dataset_name, final_psnr, final_ssim, final_psnr_aux, final_ssim_aux))

        if dataset_name == "Set5":
            self.tensorboard_writer.add_scalar('metric/PSNR_Set5', final_psnr, final_psnr_aux, epoch)
        elif dataset_name == "Set14":
            self.tensorboard_writer.add_scalar('metric/PSNR_Set14', final_psnr, final_psnr_aux, epoch)
        elif dataset_name == "B100":
            self.tensorboard_writer.add_scalar('metric/PSNR_B100', final_psnr, final_psnr_aux, epoch)
        elif dataset_name == "Manga109":
            self.tensorboard_writer.add_scalar('metric/PSNR_Manga109', final_psnr, final_psnr_aux, epoch)
            ...
            self.tensorboard_writer.add_scalar('metric/PSNR_Urban100', final_psnr, final_psnr_aux, epoch)
        else:
    ...
    def train(self):
        ...
        for epoch in range(start, total_epoch):
            for step in range(step_epoch):
                ...
                # generated_hr, generated_low, generated_high = self.network(lr)
                generated_hr = self.network(lr)
                low_freq_hr, high_freq_hr = calc_Freq(generated_hr,kernel=3) #low and high
                loss_l1  = l1(generated_hr, hr)

                loss_l1_low  = l1(low_freq_hr, low_freq_gt)
                loss_l1_high  = l1(high_freq_hr, high_freq_gt)
                # ========================================================== #
                # loss_l1_low  = l1(generated_low, low_freq_gt)
                # loss_l1_high  = l1(generated_high, high_freq_gt)
                # ========================================================== #

                loss_curr = loss_l1 + loss_l1_low + loss_l1_high 
                loss_curr.backward()
                ...
                if (step + 1) % log_frep == 0:
                    ...
                    print("[{}], Elapsed [{}], Epoch [{}/{}], Step [{}/{}], loss: {:.4f}, l1: {:.4f}, low l1: {:.4f}, high l1: {:.4f},".
                        format(self.config["version"], elapsed, epoch + 1, total_epoch, step + 1, step_epoch, 
                                loss_curr.item(),loss_l1.item(), loss_l1_low.item(), loss_l1_high.item()))
                    ...
                    self.reporter.writeTrainLog(epoch+1,step+1,
                                "loss: {:.4f}, l1: {:.4f}, low l1: {:.4f}, high l1: {:.4f}".format(loss_curr.item(), loss_l1.item(), loss_l1_low.item(), loss_l1_high.item()))
                    ...
                    if self.config["use_tensorboard"]:
                        ...
                        self.tensorboard_writer.add_scalar('data/low_l1', loss_l1_low.item(), cum_step)
                        self.tensorboard_writer.add_scalar('data/high_l1', loss_l1_high.item(), cum_step)
            ...
            if (epoch+1) % model_freq==0:
                ...
                self.__evaluation__(self.eval_loader5, self.eval_iter5, epoch+1)